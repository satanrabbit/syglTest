...
namespace ScandVault
{
    ...
    public class FileProcessor : IDisposable
    {
        ...
        private int GetStartBytePosition(ref byte[] bufferData)
        {
            ...
            byte[] searchString = Encoding.ASCII.GetBytes("Content-Type: ");
            ...
            if (tempContentTypeStart != -1)
            {
                ...
                searchString = Encoding.ASCII.GetBytes("\r\n\r\n");
            ...
            else
            {
                ...
                if (_currentBufferIndex > 0)
                {
                    ...
                    byte[] searchString2 = Encoding.ASCII.GetBytes("Content-Type: ");
                    ...
                    if (tempContentTypeStart != -1)
                    {
                        ...
                        searchString2 = Encoding.ASCII.GetBytes("Content-Type: ");
        ...
        private int GetEndBytePosition(ref byte[] bufferData)
        {
            ...
            int tempFieldSeparator = -1;

            // First see if we can find it in the current buffer batch.
            byte[] searchString = Encoding.ASCII.GetBytes(_boundary);
            tempFieldSeparator = FindBytePattern(ref bufferData, ref searchString, byteOffset);
            ...
            if (tempFieldSeparator != -1)
            {
                ...
                if (tempFieldSeparator-4 < 0)
                {
                    //TODO: Have to get the previous buffer data.
                }
                else
                {
                    return tempFieldSeparator-4;
                }
            }
            else if ((byteOffset - searchString.Length) > 0)
            {
            ...
            else
            {
                ...
                if (_currentBufferIndex > 0)
                {
                    ...
                    byte[] mergedBytes = MergeArrays(ref previousBuffer, ref bufferData);
                    // Get the byte array for the text
                    byte[] searchString2 = Encoding.ASCII.GetBytes(_boundary);
                    // Search the bytes for the searchString
                    ...
                    if (tempFieldSeparator != -1)
                    {
                        ...
                        searchString2 = Encoding.ASCII.GetBytes("\r\n\r\n");
